...
$sql = "SELECT * FROM staff WHERE universidad = '" . $_SESSION['uni'] . "';";
...
$sqlPaginacion = "SELECT * FROM staff WHERE universidad = '" . $_SESSION['uni'] . "' LIMIT $indiceInicio, $registrosPorPagina";
    ...
    <div id='center' class="main center">
        <div style="margin: 10%;">
            <div className="d-flex">
                <div>
                    <h1 style="font-weight:bold; font-size:4vmin">Inicio / Registro Staff</h1>
                    <div>
                        <a href="#" class="AddButtonv2" id="botonAdd" onclick="toggleDiv()"><i class="fa fa-plus"></i>&nbsp;&nbsp;   Agregar registro de Staff</a>
                                ...
                                <?php while ($row = $resultPaginacion->fetch_assoc()) { ?>
                                    <tr>
                                        ...
                                        <td class="iconsDesign">
                                            ...
                                            <a href="eliminar_staff.php?id=<?php echo $row['id']; ?>" class="deleteUser" data-userid="<?php echo $row['id']; ?>"><i class="fa fa-trash"></i></a>
                                        </td>
                                        <?php } ?>
                                    </tr>
                                <?php } ?>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($nombre) || empty($ape_pat) || empty($ape_mat) || empty($celular)) {
        ...
        $query = "INSERT INTO staff (universidad, nombre, ape_pat, ape_mat, celular, imagen) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($query);

        if ($stmt) {
            $stmt->bind_param("ssssis",$universidad, $nombre, $ape_pat, $ape_mat, $celular, $imagenContenido);
            if ($stmt->execute()) {
                ...
                echo "<script>
                    Swal.fire({
                        ...
                        window.location.href = 'staff.php'; // Redireccionar a la página principal
                    });
                </script>";
            } else {
