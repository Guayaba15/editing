...
$sql = "SELECT * FROM medicos WHERE universidad = '" . $_SESSION['uni'] . "';";
...
$sqlPaginacion = "SELECT * FROM medicos WHERE universidad = '" . $_SESSION['uni'] . "' LIMIT $indiceInicio, $registrosPorPagina";
    ...
    <div id='center' class="main center">
        <div style="margin: 10%;">
            <div className="d-flex">
                <div>
                    <h1 style="font-weight:bold; font-size:4vmin">Inicio / Registro Medicos</h1>
                    <div>
                        <a href="#" class="AddButtonv2" id="botonAdd" onclick="toggleDiv()"><i class="fa fa-plus"></i>&nbsp;&nbsp;   Agregar registro de Medicos</a>
                                ...
                                <table>
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <input placeholder="Cedula Profesional" name="cedula_prof" type="text">
                                        </td>
                                        <td>
                                            <label>Correo</label><br>
                                            <input placeholder="Correo" name="correo" type="text">
                                        </td>
                                        <td>
                                ...
                                <?php while ($row = $resultPaginacion->fetch_assoc()) { ?>
                                    <tr>
                                        ...
                                        <td class="iconsDesign">
                                            ...
                                            <a href="eliminar_medico.php?id=<?php echo $row['id']; ?>" class="deleteUser" data-userid="<?php echo $row['id']; ?>"><i class="fa fa-trash"></i></a>
                                        </td>
                                        <?php } ?>
                                    </tr>
                                <?php } ?>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($nombre) || empty($ape_pat) || empty($ape_mat) || empty($cedula_prof) || empty($correo) || empty($celular)) {
        // Mostrar mensaje SweetAlert de campos vacíos
        echo "<script>
            Swal.fire({
                icon: 'warning',
                title: '¡Campos vacíos!',
        ...
        $query = "INSERT INTO medicos (universidad, nombre, ape_pat, ape_mat, cedula_prof, correo, celular, imagen) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($query);

        if ($stmt) {
            $stmt->bind_param("ssssssis",$universidad, $nombre, $ape_pat, $ape_mat, $cedula_prof, $correo, $celular, $imagenContenido);
            if ($stmt->execute()) {
                ...
                echo "<script>
                    Swal.fire({
                        ...
                        window.location.href = 'medicos.php'; // Redireccionar a la página principal
                    });
                </script>";
            } else {
