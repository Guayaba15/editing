...
$sql = "SELECT * FROM universidades WHERE universidad = '" . $_SESSION['uni'] . "';";
...
$sqlPaginacion = "SELECT * FROM universidades WHERE universidad = '" . $_SESSION['uni'] . "' LIMIT $indiceInicio, $registrosPorPagina";
    ...
    <div id='center' class="main center">
        <div style="margin: 10%;">
            <div className="d-flex">
                <div>
                    <h1 style="font-weight:bold; font-size:4vmin">Inicio / Registro Universidades</h1>
                    <div>
                        <a href="#" class="AddButtonv2" id="botonAdd" onclick="toggleDiv()"><i class="fa fa-plus"></i>&nbsp;&nbsp;   Agregar registro de Universidad</a>
                        <div id="miDiv" style="display: none;">
                            ...
                            <form method="POST" enctype="multipart/form-data">
                                <table>
                                    ...
                                    <tr>
                                        <td>
                                            <label>No. Camiones</label><br>
                                            <input placeholder="No. Camiones" name="no_camiones" type="number">
                                        </td>
                                        <td>
                                            <label>No. Vans</label><br>
                                            <input placeholder="No. Vans" name="no_vans" type="number">
                                        </td>
                                        <td>
                                            <label>No. Carros</label><br>
                                            <input placeholder="No. Carros" name="no_carros" type="number">
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>
                                            <label>Hotel</label><br>
                                            <input placeholder="Hotel" name="hotel" type="text">
                                        </td>
                                        <td>
                                            <label>Requiere guia</label><br>
                                            <select name="req_guia" id="req_guia">
                                                <option value="" disabled selected>-- Seleccione una opción --</option>
                                        ...
                                        <td>
                                            <label>No. Guías</label><br>
                                            <input placeholder="No. Guías" name="no_guias" type="number">
                                        </td>
                                    </tr>
                                    <tr>
                                        <td>
                            ...
                            <tbody>
                                <?php while ($row = $resultPaginacion->fetch_assoc()) { ?>
                                    <tr>
                                        ...
                                        <td><?php echo $row['no_camiones']; ?></td>
                                        <td><?php echo $row['no_vans']; ?></td>
                                        <td><?php echo $row['no_carros']; ?></td>
                                        <td><?php echo $row['hotel']; ?></td>
                                        <td><?php echo $row['req_guia']; ?></td>
                                        <td><?php echo $row['no_guias']; ?></td>
                                        ...
                                        <td class="iconsDesign">
                                            ...
                                            <a href="eliminar_universidad.php?id=<?php echo $row['id']; ?>" class="deleteUser" data-userid="<?php echo $row['id']; ?>"><i class="fa fa-trash"></i></a>
                                        </td>
                                        <?php } ?>
                                    </tr>
                                <?php } ?>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($_POST["estado"]) || empty($_POST["clave"]) || empty($_POST["no_camiones"]) || empty($_POST["no_vans"]) || empty($_POST["no_carros"]) || empty($_POST["hotel"]) || empty($_POST["req_guia"]) || empty($_POST["no_guias"])) {
        echo '<script>
                Swal.fire({
                    icon: "warning",
    ...
    $universidad = $uni;
    $clave = $_POST["clave"];
    $no_camiones = $_POST["no_camiones"];
    $no_vans = $_POST["no_vans"];
    $no_carros = $_POST["no_carros"];
    $hotel = $_POST["hotel"];
    $req_guia = $_POST["req_guia"];
    $no_guias = $_POST["no_guias"];
        ...
        $verificar_query = "SELECT COUNT(*) as total FROM universidades WHERE universidad = '" . $_SESSION['uni'] . "';";
        $verificar_result = $conn->query($verificar_query);
        $row = $verificar_result->fetch_assoc();
        $totalRegistros = $row["total"];
        
        if ($totalRegistros > 0) {
            // Mostrar mensaje SweetAlert de registro duplicado
            echo "<script>
                Swal.fire({
                    icon: 'error',
                    title: '¡Error!',
                    text: 'Ya existe un registro en la tabla.',
                });
            </script>";
            $conn->close();
        ...
        $query = "INSERT INTO universidades (estado, universidad, clave, no_camiones, no_vans, no_carros, hotel, req_guia, no_guias, imagen) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($query);

        if ($stmt) {
            $stmt->bind_param("ssssssssss", $estado,$universidad, $clave, $no_camiones, $no_vans, $no_carros, $hotel, $req_guia, $no_guias,$imagenContenido);
            if ($stmt->execute()) {
                ...
                echo "<script>
                    Swal.fire({
                        ...
                        window.location.href = 'universidades.php'; // Redireccionar a la página principal
                    });
                </script>";
            } else {
